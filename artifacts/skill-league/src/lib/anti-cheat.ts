...
export function checkPostSpam(text: string, lastPostTs: number | null): { spam: boolean; reason?: string } {
  if (!text || text.trim().length < 3) return { spam: true, reason: 'Post too short' };
  if (text.length > 280)               return { spam: true, reason: 'Max 280 characters' };
