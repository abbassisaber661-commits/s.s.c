...
/** Sync authoritative LP value from server into localStorage without resetting other stats. */
export function syncLpFromServer(serverLp: number): void {
  const s = loadLeagueStats();
  s.lp    = serverLp;
  saveLeagueStats(s);
}
