...
export default function PvP() {
          ...
          {ARENAS.map(a => {
            ...
            return (
              <motion.button
                key={a.id}
                whileTap={{ scale: 0.97 }}
                onClick={() => { if (!isGuest && canAfford && connected) { setSelectedArena(a.id); joinQueue(a.id, a.stake); } }}
  ...
  if ((phase === 'found' || phase === 'countdown') && matchInfo) {
    ...
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-5" dir="rtl"
        style={{ background: `radial-gradient(ellipse at 50% 0%, ${arena.color}12 0%, hsl(var(--background)) 60%)` }}>
        <motion.div initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }}
          transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }}
          className="w-full max-w-sm space-y-6 text-center">

          {/* "Opponent found" badge — never reveals bot/real distinction */}
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-black"
            style={{
              background: `${arena.color}20`,
              border: `1px solid ${arena.color}50`,
              color: arena.color,
              boxShadow: `0 0 20px ${arena.color}30`,
            }}
          >
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            <motion.div
              ...
              <div className="w-14 h-14 rounded-2xl bg-indigo-500/20 flex items-center justify-center text-2xl font-black text-indigo-300 mx-auto mb-2">
                {matchInfo.playerA.name.slice(0, 2).toUpperCase()}
              </div>
              <div className="font-bold text-sm text-white truncate">{matchInfo.playerA.name}</div>
              <div className="text-xs text-white/40">Lv.{matchInfo.playerA.level} · {matchInfo.playerA.elo} ELO</div>
            </motion.div>
            ...
            {/* Player B — always looks like a real player */}
            <motion.div
              initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }}
              className="flex-1 rounded-2xl p-4"
              style={{ background: `${arena.color}12`, border: `1px solid ${arena.color}30` }}>
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl font-black mx-auto mb-2"
                style={{ background: `${arena.color}20`, color: arena.color }}>
                {matchInfo.playerB.name.slice(0, 2).toUpperCase()}
              </div>
              <div className="font-bold text-sm text-white truncate">{matchInfo.playerB.name}</div>
              <div className="text-xs text-white/40">Lv.{matchInfo.playerB.level} · {matchInfo.playerB.elo} ELO</div>
            </motion.div>
          </div>

          {/* Countdown */}
          {phase === 'countdown' && (
            <motion.div key={countdownSec}
              initial={{ scale: 2.2, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ type: 'spring', stiffness: 300, damping: 20 }}
              className="text-8xl font-black tabular-nums"
              style={{ color: arena.color, filter: `drop-shadow(0 0 20px ${arena.color}80)` }}>
              {countdownSec > 0 ? countdownSec : '🚀'}
            </motion.div>
          )}
          {phase === 'found' && (
            <motion.div className="flex items-center justify-center gap-2" animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1.2, repeat: Infinity }}>
              <div className="w-2 h-2 rounded-full" style={{ background: arena.color }} />
              <p className="text-sm font-medium text-white/50">تحضير الحلبة…</p>
              <div className="w-2 h-2 rounded-full" style={{ background: arena.color }} />
            </motion.div>
          )}
        </motion.div>
