...
export default function LeagueSelect() {
  ...
  return (
    <div className="min-h-screen bg-background p-6 flex flex-col max-w-md mx-auto relative">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold">{t('play')}</h1>
        </div>
      ...
      <div className="space-y-4">
        {(Object.keys(LEAGUES) as LeagueType[]).map((league) => {
          ...
          const canAfford = config.entryCostType === 'none' || 
          ...
          return (
            <div 
              ...
              className={`relative overflow-hidden rounded-2xl border-2 p-5 transition-all ${
                isLocked || !canAfford ? 'opacity-60 border-border bg-card/50' : 'border-border bg-card cursor-pointer hover:border-primary/50 active:scale-[0.98]'
              }`}
            >
              <div className="flex items-center gap-4 relative z-10">
                  ...
                  <div className="text-sm text-muted-foreground mt-1 flex items-center gap-2">
                    {config.entryCostType === 'none' ? (
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {config.entryCostType === 'coins' ? <Coins className="w-3 h-3 text-yellow-400" /> : <Trophy className="w-3 h-3 text-purple-400" />}
                      </span>
                    )}
                  </div>
                </div>
                <div className="text-right">
                  ...
                  <div className="font-mono font-bold text-lg">{highScores[league] || 0}</div>
                </div>
              </div>
