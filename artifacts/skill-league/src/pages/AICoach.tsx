...
function getCoachResponse(msg: string, stats: any, language: string): string {
  ...
  if (m.includes('coin') || m.includes('عملة') || m.includes('money') || m.includes('مال')) {
    return isAr
      ? `💰 أفضل طرق كسب العملات:\n\n1. اكمل التحديات اليومية (+25-40 🪙)\n2. العب PvP وارفع ELO\n3. اجمع المهمات الأسبوعية (+75-200 🪙)\n4. افتح صناديق اللوت المجانية يومياً\n5. فعّل Pi Lock لمكافآت إضافية`
      : `💰 Best ways to earn coins:\n\n1. Complete daily challenges (+25-40 🪙)\n2. Play PvP and climb ELO\n3. Finish weekly missions (+75-200 🪙)\n4. Open the free daily loot box\n5. Activate Pi Lock for bonus coins`;
  }
...
export default function AICoach() {
  const { language, level, elo, matchesPlayed, pvpWins, pvpLosses, pvpWinStreak, bestStreak, skillSpeed, skillAccuracy, skillMemory, coins, tournamentWins, dailyChallengesCompleted, xpBoostUntil } = useGame();
  const rtl = isRTL(language);

  const stats = { level, elo, matchesPlayed, pvpWins, pvpLosses, pvpWinStreak, bestStreak, skillSpeed, skillAccuracy, skillMemory, coins, tournamentWins, dailyChallengesCompleted: dailyChallengesCompleted ?? 0, xpBoostUntil: xpBoostUntil ?? null };
