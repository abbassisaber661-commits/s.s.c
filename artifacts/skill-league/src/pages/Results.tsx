...
export default function Results() {
  ...
  const getRank = () => {
    if (lastAccuracy >= 90 && lastScore >= 120) return { icon: '👑', key: 'rank_legendary' as const, color: '#FFD700' };
    if (lastAccuracy >= 80 && lastScore >= 90)  return { icon: '💎', key: 'rank_excellent' as const, color: '#60a5fa' };
    if (lastAccuracy >= 70 && lastScore >= 60)  return { icon: '⭐', key: 'rank_very_good' as const, color: '#a78bfa' };
    if (lastAccuracy >= 60)                     return { icon: '✅', key: 'rank_good' as const, color: '#34d399' };
    return                                             { icon: '💪', key: 'rank_keep_going' as const, color: '#f87171' };
  };
  const rank = getRank();
  ...
  useEffect(() => { if (lastLevelUp) setTimeout(() => playLevelUp(), 600); }, [lastLevelUp]);
  useEffect(() => { if (lastNewAchievements.length > 0) setTimeout(() => playAchievement(), 800); }, [lastNewAchievements]);
  useEffect(() => { if (lastCoinsEarned > 0) setTimeout(() => playCoin(), 400); }, [lastCoinsEarned]);

  function handleShare() {
    const msg = `🎮 ${lastScore} ${t('points')} — ${lastAccuracy}% ${t('accuracy')} — ${lastStreak} ${t('streak')}`;
    const post = createPost(username, level, fame, msg, 'text');
    addPost(post);
    setShared(true);
  ...
  return (
    <div dir={rtl ? 'rtl' : 'ltr'} className="min-h-screen bg-background flex flex-col items-center p-5 text-center overflow-y-auto relative pb-28">
      <WinAnimation show={showWinAnim && isGoodResult} label={lastLevelUp ? t('result_level_up') : '🎉'} />
      ...
      <div className="w-full max-w-sm space-y-3 mb-5 mt-2">
        ...
        {unlockedCfg && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className={`flex-1 ${rtl ? 'text-right' : 'text-left'}`}>
              <div className="font-bold text-sm" style={{ color: unlockedCfg.themeColor }}>{t('result_league_unlocked')}</div>
              <div className="text-xs text-muted-foreground">{lastUnlockedLeague}</div>
            </div>
          </motion.div>
        )}
        ...
        {lastLevelUp && (
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <div className={`flex-1 ${rtl ? 'text-right' : 'text-left'}`}>
              <div className="font-bold text-sm text-yellow-400">{t('result_level_up')} 🎉</div>
              <div className="text-xs text-muted-foreground">{t('level_label')} {level} · {getLevelTitle(level).title}</div>
            </div>
          </motion.div>
        )}
        ...
        {lastNewTrophies.map(tr => (
          <motion.div key={tr.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
            ...
            <span className="text-2xl">🏆</span>
            <div className={`flex-1 ${rtl ? 'text-right' : 'text-left'}`}>
              <div className="font-bold text-sm text-orange-400">{t('result_trophy_unlocked')}</div>
        ...
        {lastNewAchievements.map(a => (
          <motion.div key={a.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
            ...
            <div className={`flex-1 ${rtl ? 'text-right' : 'text-left'}`}>
              <div className="font-bold text-sm text-yellow-400">{t('result_achievement_new')} 🎖️</div>
              <div className="text-xs text-muted-foreground">{a.nameAr}</div>
            </div>
            {a.rewardCoins > 0 && <div className="text-yellow-400 font-bold text-sm">+{a.rewardCoins}🪙</div>}
          </motion.div>
        ))}
        ...
        {completedNames.map(c => c && (
          <motion.div key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className={`flex-1 ${rtl ? 'text-right' : 'text-left'}`}>
              <div className="font-bold text-sm text-green-400">{t('result_challenge_done')}</div>
              <div className="text-xs text-muted-foreground">+{c.rewardCoins} {t('coin_label')} · +{c.rewardElo} ELO</div>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        transition={{ type: 'spring', stiffness: 200, damping: 14, delay: 0.1 }} className="mb-3">
        <div className="text-5xl mb-1">{rank.icon}</div>
        <div className="text-sm font-black uppercase tracking-widest" style={{ color: rank.color }}>{t(rank.key)}</div>
      </motion.div>

      {/* Score */}
      <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        transition={{ type: 'spring', stiffness: 200, damping: 15, delay: 0.15 }} className="mb-5">
      ...
      {lastXpEarned > 0 && (
        <div className="w-full max-w-sm mb-4">
          <div className="flex items-center justify-between text-sm mb-1.5">
            ...
            <span className="text-muted-foreground text-xs">{t('level_label')} {level}</span>
          </div>
      ...
      <div className="grid grid-cols-3 gap-3 w-full max-w-sm mb-4">
        {[
          { value: `${lastAccuracy}%`, label: t('accuracy'),    color: '#34d399' },
          { value: lastCorrect,        label: t('correct_label'), color: '#fbbf24' },
          { value: lastStreak,         label: t('best_streak'),  color: '#fb923c' },
        ].map(stat => (
      ...
      {lastCoinsEarned > 0 && (
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <span className="text-sm text-muted-foreground">{t('coins_earned_label')}</span>
        </motion.div>
      )}
      ...
      <div className="w-full max-w-sm space-y-3">
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <button onClick={handleShare}
            ...
            {shared ? t('share_done_btn') : t('share_btn')}
          </button>
