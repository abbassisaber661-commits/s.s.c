...
export default function UserProfile() {
  ...
  const userPosts  = useMemo(() => [...apiPosts].sort((a, b) => b.timestamp - a.timestamp), [apiPosts]);
  ...
  const listItems = useMemo((): { id: string; username: string }[] => {
    ...
    <div className="min-h-screen pb-28 bg-[#F5F5F7]">
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-[#E5E5E5] shadow-sm px-4 py-3 flex items-center gap-3">
        <button
          ...
          className="p-2 rounded-xl hover:bg-[#F5F5F7] active:scale-90 transition-transform"
        >
          <ArrowLeft className="w-5 h-5 text-[#111111]" />
        </button>
        <span className="font-black text-base flex-1 truncate text-[#111111]">
          {targetUsername}
        </span>
      </div>
        ...
        <div className="max-w-md mx-auto px-4 pt-6 pb-5">
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="relative">
              <div className="w-24 h-24 rounded-full overflow-hidden shadow-md border-3 border-[#E5E5E5]">
                <Avatar username={targetUsername} size="xl" shape="rounded-full" className="w-full h-full object-cover" />
              </div>
              <div
                className="absolute -bottom-1 -right-1 px-2.5 py-0.5 rounded-full text-xs font-black shadow text-white"
            ...
            <div className="text-center space-y-1">
              <div className="text-xl font-black text-[#111111]">{targetUsername}</div>
              <span
            ...
            <div className="w-full space-y-1">
              <div className="flex justify-between text-[11px] text-[#666666]">
                <span className="flex items-center gap-1">
                  <Zap className="w-3 h-3 text-[#FFD60A]" /> Lv.{inferredLevel}
                </span>
                <span>{xpInLevel}/{XP_PER_LEVEL} XP</span>
              </div>
              <div className="h-2 rounded-full overflow-hidden bg-[#E5E5E5]">
                <motion.div
              ...
              <div className="grid grid-cols-4 divide-x divide-[#E5E5E5]">
                {[
                  ...
                  <div
                    ...
                    className={`flex flex-col items-center py-3 gap-0.5 ${onClick ? "cursor-pointer hover:bg-[#F5F5F7] active:bg-[#E5E5E5]" : ""} transition-colors`}
                  >
                    <span className="font-black text-sm tabular-nums text-[#111111]">
                      {value.toLocaleString()}
                    </span>
                    <span className="text-[9px] uppercase tracking-wide text-[#666666]">
                      {label}{onClick ? " ›" : ""}
                    </span>
                  </div>
                ))}
            ...
            {myUsername && (
              <div className="flex gap-2 w-full">
                <button
                  ...
                  className="flex-1 py-2.5 rounded-xl text-sm font-bold active:scale-95 transition-all border"
                  style={
                    isFollowing
                      ? { background: "#F5F5F7", color: "#111111", borderColor: "#E5E5E5" }
                      : { background: "#FFD60A", color: "#111111", borderColor: "#FFD60A" }
                  }
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-bold active:scale-95 transition-all bg-[#F5F5F7] text-[#111111] border border-[#E5E5E5] hover:bg-[#E5E5E5]"
                >
      ...
      {/* ── Sticky Tabs ── */}
      <div className="sticky z-10 bg-white border-b border-[#E5E5E5]" style={{ top: "57px" }}>
        <div className="flex max-w-md mx-auto">
          {TABS.map(t => (
            <button
              ...
              style={{
                borderBottomColor: tab === t.id ? "#FFD60A" : "transparent",
                color: tab === t.id ? "#111111" : "#666666",
              }}
            >
              {t.label}
            </button>
          ...
          >
              ...
              mediaPosts.length === 0
                ...
                : (
                  <div className="grid grid-cols-3 gap-1.5">
                    {mediaPosts.map(post => (
                      <div key={post.id} className="aspect-square rounded-xl overflow-hidden bg-[#E5E5E5]">
            ...
            {tab === "activity" && (
              <div className="space-y-3">
                ...
                {/* Timeline card */}
                <div className="rounded-2xl p-4 space-y-2 bg-white border border-[#E5E5E5] shadow-sm">
                  <div className="text-xs font-black uppercase tracking-wider mb-3 flex items-center gap-1.5 text-[#666666]">
                    <Calendar className="w-3.5 h-3.5 text-[#FFD60A]" /> Timeline
                  </div>
                  {[
                    { icon: <Calendar className="w-3.5 h-3.5 text-[#FFD60A]" />, label: "Joined",      value: fmtDate(backendStats?.joinedAt ?? null) },
                    { icon: <Clock     className="w-3.5 h-3.5 text-green-500" />, label: "Last Active", value: fmtRelative(backendStats?.lastActiveAt ?? null) },
                    { icon: <Zap       className="w-3.5 h-3.5 text-[#FFD60A]" />, label: "Last Post",   value: backendStats?.lastPostAt ? fmtRelative(backendStats.lastPostAt) : "No posts yet" },
                    { icon: <BookOpen  className="w-3.5 h-3.5 text-[#666666]" />, label: "Stories",     value: `${storiesCount} stories` },
                  ].map(row => (
                    <div key={row.label} className="flex items-center gap-3 py-1.5 border-b border-[#E5E5E5]/60 last:border-0">
                      {row.icon}
                      <span className="text-xs flex-1 text-[#666666]">{row.label}</span>
                      <span className="text-xs font-bold text-[#111111]">{row.value}</span>
                    </div>
                  ))}
                </div>

                {/* Social stats grid */}
                <div className="rounded-2xl p-4 space-y-3 bg-white border border-[#E5E5E5] shadow-sm">
                  <div className="text-xs font-black uppercase tracking-wider flex items-center gap-1.5 text-[#666666]">
                    <BarChart2 className="w-3.5 h-3.5 text-[#FFD60A]" /> Social Stats
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { label: "Posts",          value: postsCount,                          color: "#111111", clickable: false },
                      { label: "Stories",        value: storiesCount,                        color: "#666666", clickable: false },
                      { label: "Likes Received", value: totalLikes,                          color: "#EF4444", clickable: false },
                      { label: "Comments Got",   value: backendStats?.commentsReceived ?? 0, color: "#22C55E", clickable: false },
                      { label: "Followers",      value: followers,    color: "#111111", clickable: true, list: "followers" as ListType },
                      { label: "Following",      value: following,    color: "#111111", clickable: true, list: "following" as ListType },
                      { label: "Friends",        value: friendsCount, color: "#FFD60A", clickable: true, list: "friends"   as ListType },
                    ].map(s => (
                      <div
                        key={s.label}
                        onClick={s.clickable ? () => setActiveList(s.list!) : undefined}
                        className={`rounded-xl p-3 text-center border border-[#E5E5E5] ${s.clickable ? "cursor-pointer hover:bg-[#F5F5F7]" : ""} transition-colors bg-white`}
                      >
                        <div className="text-xl font-black tabular-nums" style={{ color: s.color }}>
                          {typeof s.value === "number" ? s.value.toLocaleString() : s.value}
                        </div>
                        <div className="text-[10px] mt-0.5 text-[#666666]">
                          {s.label}{s.clickable ? " ›" : ""}
                        </div>
                      </div>
                    ))}
                ...
                {friendsList.length > 0 && (
                  <div className="rounded-2xl p-4 space-y-3 bg-white border border-[#E5E5E5] shadow-sm">
                    <div className="flex items-center justify-between">
                      <div className="text-xs font-black uppercase tracking-wider flex items-center gap-1.5 text-[#666666]">
                        <Users className="w-3.5 h-3.5 text-[#FFD60A]" /> Friends ({friendsCount})
                      </div>
                      {friendsCount > 4 && (
                        <button
                          onClick={() => setActiveList("friends")}
                          className="text-xs font-bold text-[#111111] hover:text-[#666666] transition-colors"
                        >
                          See All ›
                        </button>
                      )}
                    ...
                    <div className="grid grid-cols-4 gap-2">
                      {friendsList.slice(0, 4).map(f => (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-semibold text-center truncate w-full text-[#111111]">
                            {f.username}
                          </span>
                        </button>
                      ))}
...
function MiniPostCard({
  ...
  index: number;
  leagueColor: string;
  meta: { views: number; shares: number };
  commentCount: number;
  ...
  return (
    <motion.div
      ...
      className="rounded-2xl p-4 space-y-2.5 bg-white border border-[#E5E5E5] shadow-sm"
    >
      <div className="flex items-center justify-between">
        <span className="text-[11px] text-[#666666]">{getPostAge(post.timestamp)}</span>
        {post.boosted && <span className="text-[10px] text-[#FFD60A] font-bold">🔥 Boosted</span>}
      </div>
      ...
      <div className="flex items-center gap-3 pt-1 border-t border-[#E5E5E5]">
...
function ListSheet({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="relative w-full max-h-[75vh] rounded-t-3xl flex flex-col overflow-hidden bg-white shadow-2xl"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-[#E5E5E5]">
          <span className="font-black text-base text-[#111111]">
            {label} <span className="text-sm font-normal text-[#666666]">({allCount})</span>
          </span>
          <button
            onClick={onClose}
            className="p-2 rounded-xl hover:bg-[#F5F5F7] active:scale-90 transition-transform"
        ...
        <div className="px-4 py-2.5 border-b border-[#E5E5E5]">
          <div className="flex items-center gap-2 rounded-xl px-3 py-2 bg-[#F5F5F7] border border-[#E5E5E5]">
            <Search className="w-4 h-4 flex-shrink-0 text-[#666666]" />
            <input
              value={search}
              onChange={e => onSearch(e.target.value)}
              placeholder={`Search ${label.toLowerCase()}…`}
              className="flex-1 bg-transparent text-sm outline-none text-[#111111] placeholder-[#666666]"
              autoFocus
            />
            {search && (
          ...
          {loading ? (
            ...
            items.map(u => (
              <button
                ...
                className="flex items-center gap-3 w-full p-3 rounded-xl active:scale-[0.98] transition-all hover:bg-[#F5F5F7]"
              >
                <Avatar username={u.username} size="md" shape="rounded-full" />
                <div className="flex-1 text-left min-w-0">
                  <div className="text-sm font-bold truncate text-[#111111]">{u.username}</div>
                </div>
                <ChevronRight className="w-4 h-4 flex-shrink-0 text-[#666666]" />
              </button>
            ))
          )}
        </div>
