...
router.post("/marketplace", async (req, res) => {
  try {
    ...
    if (!seller.length) { res.status(404).json({ error: "player_not_found" }); return; }
    if (!seller[0].ownedItems.includes(itemId)) { res.status(400).json({ error: "item_not_owned" }); return; }
