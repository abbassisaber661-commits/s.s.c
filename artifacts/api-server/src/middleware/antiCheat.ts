...
export async function trackIpFingerprint(
  ...
  try {
    await db.execute(
      sql`INSERT INTO ip_fingerprints (ip_address, device_fingerprint, player_id, first_seen_at, last_seen_at)
          VALUES (${ip}, ${fingerprint}, ${playerId}, NOW(), NOW())
          ON CONFLICT (ip_address, player_id) DO UPDATE SET last_seen_at = NOW(), device_fingerprint = ${fingerprint}`,
    );
  } catch { /* non-critical */ }
}
...
export async function detectMultiAccount(ip: string, playerId: string): Promise<boolean> {
  try {
    const result = await db.execute(
      sql`SELECT COUNT(DISTINCT player_id) as cnt FROM ip_fingerprints
          WHERE ip_address = ${ip} AND player_id != ${playerId}`,
...
export async function detectSuspiciousWinRate(playerId: string): Promise<boolean> {
  try {
    const result = await db.execute(
      sql`SELECT matches_played, matches_won FROM players WHERE id = ${playerId}`,
...
export async function detectRapidRegistration(ip: string): Promise<boolean> {
  try {
    const result = await db.execute(
      sql`SELECT COUNT(*) as cnt FROM players
          WHERE id IN (SELECT DISTINCT player_id FROM ip_fingerprints WHERE ip_address = ${ip})
...
export async function logSuspicious(
  ...
  try {
    await db.execute(
      sql`INSERT INTO suspicious_activity (player_id, ip_address, device_fingerprint, type, severity, details)
          VALUES (${playerId}, ${ip}, ${fingerprint}, ${type}, ${severity}, ${JSON.stringify(details)})`,
    );
  } catch { /* non-critical */ }
}
...
export async function logAudit(
  ...
  try {
    await db.execute(
      sql`INSERT INTO audit_logs (player_id, action, entity, entity_id, old_value, new_value, ip_address, user_agent, status)
          VALUES (${playerId}, ${action}, ${entity}, ${entityId}, ${JSON.stringify(oldVal)}, ${JSON.stringify(newVal)}, ${ip}, ${ua}, ${status})`,
    );
  } catch { /* non-critical */ }
}
